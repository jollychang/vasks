...
def _get_access_token():
    ...
    if r.status_code != 200:
        return None
    repo_token_list = [t['token'] for t in r.json if u'repo' in t['scopes'] and u'repo:status' in t['scopes'] and u'public_repo' in t['scopes']]
    if len(repo_token_list) > 0:
        return repo_token_list[0]
    r = requests.post(GITHUB_API_BASE_URL+'/authorizations', data='{"scopes":["repo", "repo:status", "public_repo"]}',auth=(GITHUB_USER, GITHUB_PASSWORD))
...
def set_commit_status(owner, repo, sha, state, target_url, description):
    token = _get_access_token()
    if token is None:
        init.logger.info('cant get access_token. set status for  %s/%s/statuses/%s failed' % (owner, repo, sha))
        return None
    headers = {'Authorization': 'Bearer %s' % token}
    payload = json.dumps({
                "state": state,
    ...
    r = requests.post('%s/repos/%s/%s/statuses/%s' % (GITHUB_API_BASE_URL, owner, repo, sha), headers=headers, data=payload)
    if r.status_code != 201:
        return None
    return r.json['id']
